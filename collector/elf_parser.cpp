...
uint8_t *Elf_parser::get_memory_map() {
    ...
    //printf("m_mmap_program = %llx,header->e_entry= %llx,header->e_ehsize = %llx,header->e_phentsize = %llx\n",m_mmap_program,header->e_entry,header->e_ehsize,header->e_phentsize);
    return m_mmap_program;
}

uint8_t* Elf_parser::get_phy_base(){
    std::vector<section_t> sections = get_sections();
    uint64_t init_addr,init_offset;
    for(int i=0;i<sections.size();i++){
        if(sections[i].section_name == ".init"){
            init_addr = sections[i].section_addr;
            init_offset = sections[i].section_offset;
            break;
        }
    }
    uint8_t* phy_base = m_mmap_program - init_addr + init_offset;
    printf("phy_base=%llx,m_mmap_program=%llx,init_addr=%llx,init_offset=%llx\n",phy_base,m_mmap_program,init_addr,init_offset);
    printf("elf_base ");
    for(int i=0;i<16;i++){
        printf("%02x ",*((unsigned char*)(m_mmap_program+i)));
    }
    printf("\n");

    //debug
    printf("init_addr=%llx,init_offset=%llx,print_addr=%llx\n",init_addr,init_offset,m_mmap_program+0x400);
    printf("elf_init_base ");
    for(int i=0;i<16;i++){
        printf("%02x ",*((unsigned char*)(phy_base+i+init_addr)));
    }
    printf("\n");
    /*printf("elf_init_base ");
    for(int i=0;i<16;i++){
        printf("%02x ",*((unsigned char*)(m_mmap_program+i+0x400)));
    }
    printf("\n");*/
...
uint64_t Elf_parser::get_fini(){
    std::vector<section_t> sections = get_sections();
    uint64_t fini_addr,fini_offset;
    for(int i=0;i<sections.size();i++){
        if(sections[i].section_name == ".fini"){
            fini_addr = sections[i].section_addr;
            fini_offset = sections[i].section_offset;
            break;
        }
    }
    return fini_addr;
